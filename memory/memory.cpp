...
#include <vector>
#include <iostream>
#include "../math/xor.h" // Verifica se o caminho para xor.h está correto no teu projeto

// Estruturas necessárias para ler o cabeçalho PE
...
uintptr_t sig_scan(const char* module, const char* pattern)
{
	...
	// [CORREÇÃO CRÍTICA]
	// Verifica se o módulo existe ANTES de tentar ler o dosHeader.
	// Isto resolve o erro: Desreferenciamento de ponteiro NULL ''dosHeader'' e ''scanBytes''
	if (!moduleAdress) return 0;
...
// Nota: Certifica-te que 'getasynckeyState_address' não está duplicado noutro lugar
auto getasynckeyState_address = sig_scan("GameOverlayRenderer64.dll", "40 53 48 83 EC 20 80 3D ? ? ? ? ? 8B D9");
using GetAsyncKeyState_t = short(__fastcall*)(int);

short steam_keybind(const int key)
{
	if (!getasynckeyState_address) return 0; // Proteção extra
